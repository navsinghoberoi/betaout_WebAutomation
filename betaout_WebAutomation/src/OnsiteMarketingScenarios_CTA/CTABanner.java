...
public class CTABanner {
	...
	//	String name = "Auto_ctabanner"+" "+System.currentTimeMillis();
		String name = prop.getProperty("CTABannerName") + System.currentTimeMillis();
 	...
 	//	 wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[text() = 'Save & Next']")));
		 wait.until(ExpectedConditions.visibilityOfElementLocated(design.saveandnext));			 
	...
	//	String weburl = "CTABanner";
		String weburl = prop.getProperty("CTABannerURL");
	...
	//	String ActiveInstanceName = driver.findElement(By.xpath("(//a[@class = 'camp-name'])[1]")).getText();
		String ActiveInstanceName = driver.findElement(active_obj.ActiveInstanceName).getText();
	...
	//	driver.navigate().refresh();
		...
		//Start changing locators from here
		
		
		// Switch to iframe

	    driver.switchTo().frame(driver.findElement(By.xpath("//*[contains(@id,'iframeBanner')]")));
	    String BannerText = driver.findElement(By.xpath("(//*[text() = 'HEADPHONES'])")).getText();
	    System.out.println(BannerText);
			
		s_assert.assertEquals("HEADPHONES", BannerText);
		s_assert.assertAll();
