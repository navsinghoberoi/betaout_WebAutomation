 ...
 * @author - Navpreet
 */
class ViewedAlsoViewed_CTAPullout {
	...
	public void ViewedAlsoViewed_CtaPullout() throws Exception{
...
*/		
		...
		template_obj.PulloutClick();
		
	//	template_obj.CTAPopuptemplateClick();      // Select popup template
		
		driver.findElement(By.xpath("(//*[@data-tid = '407'])")).click(); // Select Recommendation Popup3
		...
		String weburl = "CTAPulloutwithMachineLearning";
	...
	//	driver.findElement(By.cssSelector("body")).sendKeys(Keys.CONTROL + "t");   	// Open new tab
	    ...
	    String PulloutText = driver.findElement(By.xpath("//*[text() = 'HI ARUN']")).getText();
	    System.out.println(PulloutText);
		
	    List<WebElement> total =  driver.findElements(By.xpath("//*[@class = 'recom_edit_title']"));
	    System.out.println("Total number of products on recommendation pullout = "+total.size());
...
/*	    for(int i=0;i<4;i++)
	    ...
	    */
		...
		s_assert.assertEquals("HI ARUN", PulloutText);
		s_assert.assertAll();
		
		System.out.println("Recommendation Pullout is present on the URL");
		Thread.sleep(5000);
